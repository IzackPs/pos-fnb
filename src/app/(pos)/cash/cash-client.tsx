    ...
    <div className="h-full overflow-y-auto p-6 space-y-6">
      ...
      <Tabs defaultValue="flows">
        ...
        <TabsContent value="flows" className="mt-4">
          <div className="section-amber overflow-hidden"><table className="w-full text-sm"><thead><tr className="bg-gray-50 border-b border-gray-200"><th className="text-left p-4">Ngày</th><th className="text-left p-4">{t.settings.type}</th><th className="text-left p-4">{t.cash.category}</th><th className="text-left p-4">{t.cash.description}</th><th className="text-right p-4">{t.order.amount}</th></tr></thead>
            <tbody>{flows.map(f => (<tr key={f.id} className="border-b border-gray-100"><td className="p-4">{new Date(f.createdAt).toLocaleTimeString("vi-VN", { hour: "2-digit", minute: "2-digit" })}</td>
              ...
              <td className={`p-4 text-right font-mono font-bold ${f.type === "INCOME" ? "text-emerald-600" : "text-red-500"}`}>{f.type === "EXPENSE" ? "-" : "+"}{fmt(f.amount)}đ</td></tr>))}</tbody></table>
            {flows.length === 0 && <p className="text-center text-gray-400 py-12">{t.reports.noData}</p>}</div>
        </TabsContent>
        <TabsContent value="register" className="mt-4">
          <div className="section-amber overflow-hidden"><table className="w-full text-sm"><thead><tr className="bg-gray-50 border-b border-gray-200"><th className="text-left p-4">{t.inventory.date}</th><th className="text-left p-4">Người mở</th><th className="text-right p-4">{t.cash.openingBalance}</th><th className="text-right p-4">{t.cash.closingBalance}</th><th className="text-right p-4">{t.cash.expectedBalance}</th><th className="text-right p-4">{t.cash.discrepancy}</th><th className="text-left p-4">TT</th></tr></thead>
            <tbody>{registers.map(r => (<tr key={r.id} className="border-b border-gray-100"><td className="p-4 font-semibold">{new Date(r.openingAt).toLocaleDateString("vi-VN")}</td><td className="p-4">{r.user?.name}</td>
              <td className="p-4 text-right font-mono">{fmt(r.openingBalance)}đ</td><td className="p-4 text-right font-mono">{r.closingBalance ? fmt(r.closingBalance) + "đ" : "—"}</td><td className="p-4 text-right font-mono">{r.expectedBalance ? fmt(r.expectedBalance) + "đ" : "—"}</td>
              <td className={`p-4 text-right font-mono font-bold ${(r.discrepancy ?? 0) !== 0 ? "text-red-500" : "text-emerald-600"}`}>{r.discrepancy !== null ? fmt(r.discrepancy) + "đ" : "—"}</td>
